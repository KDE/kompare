	...
	, m_destination( destination )
{
	...
	// Find a common root.
	KURL root( sourceURL );
	while( !root.isMalformed() && !root.isParentOf( destinationURL ) ) {
		root = root.upURL();
	}
...
void KDiffSaveOptionsWidget::updateCommandLine()
{
	...
	switch( static_cast<Format>( m_FormatBG->id( m_FormatBG->selected() ) ) ) {
	case Unified :
...
//	m_settings->m_allText = m_AllTextCB->isChecked();
	...
	m_settings->m_format = static_cast<Format>( m_FormatBG->id( m_FormatBG->selected() ) );
