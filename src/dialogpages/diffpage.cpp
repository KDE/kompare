...
void DiffPage::addDiffTab()
{
    ...
    m_diffURLRequester = new KUrlRequester(page);
    ...
    layout->addRow(i18nc("@label", "Diff Program:"), m_diffURLRequester);
...
void DiffPage::addFormatTab()
{
    ...
    auto radioButtons = new QWidget(page);
    layout->addRow(i18nc("@label", "Output Format:"), radioButtons);
    QVBoxLayout* bgLayout = new QVBoxLayout(radioButtons);

    auto radioButton = new QRadioButton(i18nc("@option:radio format type", "Context"), radioButtons);
    m_modeButtonGroup->addButton(radioButton, KompareDiff2::Context);
    bgLayout->addWidget(radioButton);

    radioButton = new QRadioButton(i18nc("@option:radio format type", "Normal"),  radioButtons);
    m_modeButtonGroup->addButton(radioButton, KompareDiff2::Normal);
    bgLayout->addWidget(radioButton);

    radioButton =  new QRadioButton(i18nc("@option:radio format type", "Unified"), radioButtons);
    m_modeButtonGroup->addButton(radioButton, KompareDiff2::Unified);
    bgLayout->addWidget(radioButton);

    auto whatsthis = new QLabel(i18nc("@info:whatsthis", "Select the format of the output generated by diff. Unified is the one that is used most frequently because it is very readable."), page);
    whatsthis->setFont(QFontDatabase::systemFont(QFontDatabase::SmallestReadableFont));
    whatsthis->setMaximumWidth(800);
    whatsthis->setWordWrap(true);
    layout->addRow(QString(), whatsthis);
    layout->addItem(new QSpacerItem(0, 12));

    // #lines of context (loc)
    m_locSpinBox = new QSpinBox(page);
    ...
    layout->addRow(i18nc("@label:spinbox", "Number of context lines:"), m_locSpinBox);
